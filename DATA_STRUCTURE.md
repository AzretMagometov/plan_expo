...
### Анализ рефлексии через ИИ

**Использование:**
После заполнения рефлексии сообщи ИИ-коучу: "Готово, проанализируй рефлексию за [дата]"

**Что делает ИИ:**
1. Читает файл рефлексии из `reflections/daily/YYYY/MM/YYYY-MM-DD.md`
2. Анализирует выполнение операций, тактики и доказательств идентичности
3. Выявляет критические события (авария, потеря дохода и т.д.)
4. Генерирует секцию "Комментарии ИИ-системы" с анализом и рекомендациями
5. Обновляет файл рефлексии, добавляя комментарии
6. При необходимости обновляет файлы целей (метрики, критические события)

**Процесс:**
- ИИ использует `prompts/reflection_management.md` для детального анализа
- При обнаружении критических событий использует `prompts/goal_change_tracking.md`
- Все изменения документируются в истории целей
